...
const findByRole = async (req, res, next) => {
    try {
        const users = userService.findUsersByRole(req.body.roles);
...
const updateUser = async (req, res, next) => {
    try {
        ...
        if (user.role === CANDIDATE || user.role === ADMIN) throw errorHandler.badRequest();
...
module.exports = { signin, findByRole, inviteCandidate, activateUser, inviteReviewer, updateUser, deleteUser };